...
//----------------------------
// UNOFFICIAL MODS (SAR):
// 05dec2000-- Added cell state flag GVIS_VIRTUALTITLETIP.  This serves as a 
//             signal to set GV_ITEM.strText to the title tip for a cell 
//             rather than the cell's contents.  Along with the modifications 
//             to CGridCtrl::GetCell() and CGridCtrl::OnMouseMove, it provides 
//             a mechanism for putting unique info in a cell's title tip when 
//             running the grid in virtual mode.
// 04nov2002-- Modified Draw() to handle possibility that a fixed cell has the focus, since CGridCtrl now allows 
//             that possibility as a settable feature.
// 05dec2002-- Fixed bug in Draw() to clean up framing of fixed cells on the same row or col as the focus 
//             cell -- when the focus cell was a fixed cell, existing code broke.
//
/////////////////////////////////////////////////////////////////////////////
...
// of bounds.
// 04nov2002(SAR)-- CGridCtrl now permits a fixed cell to be the grid's focus cell.  Drawing code here has been 
// modified to handle this possibility.
// 05dec2002(SAR)-- Further mod to fix the framing of fixed cells on the same row or col as the focus cell...
//
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if ( IsFocused() || IsDropHighlighted() )
    {
        ...
        if( IsFixed() || pGrid->GetFrameFocusCell() )                         // 04nov2002(SAR)-- ALWAYS frame a fixed 
        {                                                                     // focus cell
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        // As above, always show current location even in list mode so that we know where the cursor is at.
        // 05dec2002(SAR):  If fixed cell is in same row as the focus cell, only highlight it when it's in a fixed col; 
        // likewise, if fixed cell is in same col as focus cell, only highlight it when it's in a fixed row.
        //
        BOOL bHiliteFixed = pGrid->GetTrackFocusCell() && pGrid->IsValid(FocusCell) &&
            ( (FocusCell.row == nRow && nCol < pGrid->GetFixedColumnCount()) ||
              (FocusCell.col == nCol && nRow < pGrid->GetFixedRowCount()) );

        if( FocusCell.row == nRow && FocusCell.col == nCol )            // 04nov2002(SAR) if the fixed cell IS the 
           ;                                                            // focus cell, we already framed it above!!
        else if (bHiliteFixed)                                          // if fixed cell on same row or col as focus 
        {                                                               // cell, highlight it
            rect.right++; rect.bottom++;
