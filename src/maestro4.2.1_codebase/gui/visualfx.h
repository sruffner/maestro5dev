...
// class TTabStatic
//
// This class is a simple derivative of CStatic that truncates text that does not fit within the control's bounding 
// rect.  IT ASSUMES single-line text and WILL FAIL in multi-line situations!!!
//===================================================================================================================== 

class TTabStatic : public CStatic
{
public:
   TTabStatic() {}
   ~TTabStatic() {}
...
protected:
   afx_msg void OnPaint();                   // special handling of text string that does not fit in bounding rect
...
private:
  ...
  TTabStatic *m_pCaption;   // Caption of this tab
...
protected:
  ...
  CFont m_Font;
   CFont m_selectedFont;         // 15mar2004(sar):  font for the selected tab
   CBrush m_brushSelTabBkg;      // 17mar2004(sar):  brush for selected tab bkg
...
class TVTabPane : public CFormView 
{
   DECLARE_DYNAMIC(TVTabPane)

protected:
   TVTabPane(LPCTSTR lpszTemplateName);
   TVTabPane(UINT nIDTemplate);

protected:
   afx_msg int OnCreate( LPCREATESTRUCT lpcs );
   DECLARE_MESSAGE_MAP()

   BOOL UpdateCaption( LPCTSTR szCaption );
   BOOL BringToFront();
};
