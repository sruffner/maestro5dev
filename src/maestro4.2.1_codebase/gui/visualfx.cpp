...
//===================================================================================================================== 
// SUMMARY OF MODIFICATIONS STARTING 11JUL2001 (saruffner)
//
// 11jul2001-- Added class TTabStatic, a derivative of CStatic that truncates its text to fit within its current 
//             boundary rect.  The middle of the text string is truncated and replaced by an ellipsis.  TTabItem was 
//             modified to use TTabStatic instead of CStatic.
// 15mar2004-- Modified TTabWnd so that the tab label of the currently selected tab is rendered in a bolder version of 
//             the same font.  This, of course, assumes that the standard font created by createFont() is not bold 
//             already.  See createFont(), drawSelTab***().
// 17mar2004-- Further modified TTabWnd so that the rectangle immediately surrounding the text label of the currently 
//             selected tab is painted using the system tooltip bkg color -- further drawing the user's attention to 
//             what is currently at the front of the tab window.
//          -- Fixed calculation of tab text length in the TTabWnd methods that render the tabs.
//          -- Changed TTabStatic::OnPaint() so that it just draws the first portion of the text with an end ellipsis 
//             when the text does not fit in the text rect.
//          -- Adjusted spacing between icon and tab text in TTabWnd methods that render the tabs.
//          -- When a tab is NOT selected, its maximum length is half that of the selected tab.  This feature was 
//             added to reduce the real estate eaten up by the tabs, since TTabWnd does not support scrolling the 
//             tab area when it gets to be longer than the tab window's width.
...
BEGIN_MESSAGE_MAP(TTabStatic, CStatic)
  ON_WM_PAINT()
END_MESSAGE_MAP()

//=== OnPaint ========================================================================================================= 
//
//    Here we customize drawing of the static control text, clipping the string with an end ellipsis ("...") if 
//    necessary to make it fit within the current boundary rect of the control.
//
void TTabStatic::OnPaint()
{
   CPaintDC dc( this );
   ...
   int rWidth = rClient.Size().cx;

   CFont* pOldFont = dc.SelectObject( GetFont() );
   dc.SetBkMode( TRANSPARENT );

   dc.DrawText( strText, &rClient, DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS );
/*
   CString strAdj = strText;
   TEXTMETRIC tm;
   ...
   while( szText.cx > rWidth )
   {
      int nCharsToDrop = nExtra + (szText.cx - rWidth)/tm.tmAveCharWidth;
      if( nCharsToDrop >= len )
         strAdj = _T("");
      else
      {
         int iHalfRem = (len - nCharsToDrop) / 2;
         strAdj.Format( "%s...%s", strText.Left( iHalfRem ), 
                        strText.Right( len - nCharsToDrop - iHalfRem ) );
      }
      szText = dc.GetTextExtent( strAdj );
      ++nExtra;
   }
   dc.DrawText( strAdj, &rClient, DT_SINGLELINE|DT_VCENTER|DT_CENTER );
*/
...
#define TAB_MAXLEN_SEL    200   // 17mar2004(sar): Different max tab lengths for selected vs unselected tabs.
#define TAB_MAXLEN_UNSEL  80
#define TAB_ICON          16
#define TAB_ICON_X        4      // 17mar2004(sar):  Adjusted. Represents gap between icon and left edge of tab
#define TAB_ICON_Y        2
...
TTabWnd::TTabWnd()
{
  ...
  m_PenDGray2.CreatePen(PS_SOLID, 2, ::GetSysColor(COLOR_BTNSHADOW));
   ...
   // 17mar2004:  use the bkg color for tooltip controls as the bkg of the text label in a selected tab
   m_brushSelTabBkg.CreateSolidBrush( ::GetSysColor(COLOR_INFOBK) );
}
...
// 15mar2004(sar):  Create a second font of heavier weight to render label of the selected tab.
void TTabWnd::createFont()
{
   ...
   LONG lFontWt = metrics.lfStatusFont.lfWeight + 300;
   if( lFontWt < FW_REGULAR ) lFontWt = FW_BOLD;
   else if( lFontWt > 1000 ) lFontWt = 1000;
   metrics.lfStatusFont.lfWeight = lFontWt;
   m_selectedFont.CreateFontIndirect( &metrics.lfStatusFont );
}
...
// Set font
// 15mar2004(sar):  Derives a second, more heavily weighted font from specified font to render label of selected tab.
...
// 15mar2004(sar):  Use bolder font for the label of a selected tab
// 17mar2004(sar):  Use a tooltip bkg color for immediate bkg of selected tab's text label.  Fixed calc of 
// text extent -- need to select the correct font into DC before using GetTextExtent().
int TTabWnd::drawSelTabTop(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ASSERT(pItem);
  ASSERT(pDC);

   // calculate text extent -- use the font which will appear in the text label
   CFont *pOldFont = pDC->SelectObject( &m_selectedFont );
   CString str = pItem->GetText();
   CSize textSize = pDC->GetTextExtent(str);
   pDC->SelectObject( pOldFont );

   textSize.cx += 4;
   if (textSize.cx > TAB_MAXLEN_SEL)
      textSize.cx = TAB_MAXLEN_SEL;
  ...
  pDC->LineTo(client.right, y+TABSEL_HEIGHT+1);
   ...
   // paint tab bkg in gray 
   pDC->FillSolidRect(x+3,y+3,textSize.cx+TAB_SPACE-6, TABSEL_HEIGHT, ::GetSysColor(COLOR_BTNFACE));

  if (pItem->m_hIcon) {
    ::DrawIconEx(pDC->m_hDC,x+TAB_ICON_X,y+TAB_ICON_Y,pItem->m_hIcon,16,16,0,NULL,DI_NORMAL);
  }

   // fill bkg of tab text rect with tooltip bkg color
   CRect rect(CPoint(x+nDeltaWidth+TAB_SPACE/2, y+(TAB_HEIGHT-textSize.cy)/2+1), 
              CSize(textSize.cx-nDeltaWidth, textSize.cy));
   pDC->FillRect( &rect, &m_brushSelTabBkg );
   pItem->SetFont(&m_selectedFont);
   pItem->SetRect(rect);
...
int TTabWnd::drawSelTabBottom(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  // paint bkg in gray
  pDC->FillSolidRect(x+3,y,textSize.cx+TAB_SPACE-6, TABSEL_HEIGHT-2, ::GetSysColor(COLOR_BTNFACE));
...
// 17mar2004(sar):  Fixed calc of text extent -- need to select the correct font into DC before using GetTextExtent().
int TTabWnd::drawTabTop(CDC *pDC, int x, CRect& client, TTabItem *pItem)
{
  ...
  ASSERT(pDC);
   ...
   textSize.cx += 4;
   if (textSize.cx > TAB_MAXLEN_UNSEL)
      textSize.cx = TAB_MAXLEN_UNSEL;
...
//
// DESCRIPTION:
// This class is designed solely for use as a tab pane of the Visual Framework's tab window class, TTabWnd.  Unlike 
// TVisualFormView, it does not automatically enable/disable all child controls when the window is enabled/disabled -- 
// which is not always a desired behavior.  More importantly, it provides two convenience functions by which derived 
// classes can "communicate" with the parent TTabWnd:
//    UpdateCaption() -- tells parent to update caption of the tab pane associated with this view.
//    BringToFront() -- tell parent to bring the tab pane associated with this view to the front. 
//
// REVISION HISTORY:
// 26mar2001-- Created as a supplement to the Visual Framework library.
//
//===================================================================================================================== 

IMPLEMENT_DYNAMIC(TVTabPane, CFormView)

BEGIN_MESSAGE_MAP(TVTabPane, CFormView)
   ON_WM_CREATE()
END_MESSAGE_MAP()

TVTabPane::TVTabPane( LPCTSTR lpszTemplateName ) : CFormView( lpszTemplateName )
{
}

TVTabPane::TVTabPane( UINT nIDTemplate ) : CFormView( nIDTemplate )
{
}

int TVTabPane::OnCreate( LPCREATESTRUCT lpcs )                    // this class MUST be parented by a TTabWnd!!
{
   
   TTabWnd* pParent = (TTabWnd*) FromHandle( lpcs->hwndParent );
   if( pParent->IsKindOf( RUNTIME_CLASS(TTabWnd) ) ) 
      return( CFormView::OnCreate( lpcs ) );
   else
      return( -1 );
}

BOOL TVTabPane::UpdateCaption( LPCTSTR szCaption )
{
   TTabWnd* pParent = (TTabWnd*) GetParent();
   ASSERT( pParent );
   ASSERT_KINDOF( TTabWnd, pParent );
   int nIndex = pParent->GetTabIndex( this );
   if( nIndex >= 0 )
      pParent->SetTabLabel( nIndex, szCaption );
   return( BOOL(nIndex>=0) );
}

BOOL TVTabPane::BringToFront()
{
   TTabWnd* pParent = (TTabWnd*) GetParent();
   ASSERT( pParent );
   ASSERT_KINDOF( TTabWnd, pParent );
   int nIndex = pParent->GetTabIndex( this );
   if( nIndex >= 0 )
      return( pParent->SetActivePane( nIndex, FALSE ) );
   else
      return( FALSE );
}
