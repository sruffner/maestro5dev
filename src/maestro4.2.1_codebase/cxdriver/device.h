...
//===================================================================================================================== 
private:
   CDevice( const CDevice& src );                        // no copy constructor or assignment operator defined
...
public:
   ...
   BOOL RTFCNDCL IsPci()                                 // TRUE if device object represents a device on PCI bus
   {
      return( BOOL((m_devInfo.flags & DF_PCI) != 0) );
   }
   BOOL RTFCNDCL IsIsa() { return( !IsPci() ); }         // TRUE if device object represents a device on ISA bus
   BOOL RTFCNDCL IsTIDsp()                               // TRUE if dev obj represents a TI DSP device
   {
      return( BOOL((m_devInfo.flags & (DF_TIC6x|DF_TIC4x)) != 0) );
   }
   BOOL RTFCNDCL IsTIC6x()                               // TRUE if dev obj represents a TI TMS320C6x-series DSP dev
   {
      return( BOOL((m_devInfo.flags & DF_TIC6x) != 0) );
   }

   int RTFCNDCL GetPciBus()                              // retrieve PCI bus # (returns -1 if ISA or no device found)
   {
      return( IsPci() ? m_nPciBus : -1 );
   }
   ULONG RTFCNDCL GetPciSlot()                           // retrieve PCI slot # (returns 0 if ISA or no device found)
   {
      return( IsPci() ? m_ulPciSlot : 0 );
   }
   DWORD RTFCNDCL GetPciBaseAddressReg( int iReg )       // retrieve a base addr reg from PCI config info
   {
      if( iReg >= 0 && iReg < 4 ) return( m_pciInfo.u.type0.BaseAddresses[iReg] );
      else return( 0 );
   }
   WORD RTFCNDCL GetSubVendor()                          // retrieve subvendor ID for this PCI device
   { 
      return( IsPci() ? m_pciInfo.u.type0.SubVendorID : 0 );
   }
   WORD RTFCNDCL GetSubSystem()                          // retrieve subsystem ID for this PCI device
   { 
      return( IsPci() ? m_pciInfo.u.type0.SubSystemID : 0 );
   }
   BOOL RTFCNDCL GetPciConfig( PCI_COMMON_CONFIG* pCfg ) // retrieve the entire PCI config structure
   {
      if( IsPci() ) *pCfg = m_pciInfo;
      return( IsPci() );
   }
   BOOL RTFCNDCL HasInterruptHandler()                   // TRUE if an ISR is currently attached to dev interrupts
   {
      return( BOOL(m_hInterrupt != NULL) );
   }
   LPCSTR RTFCNDCL GetLastDeviceError()                  // msg describing last device error (empty string if no error)
   {
