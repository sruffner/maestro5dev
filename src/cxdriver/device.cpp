...
//
constexpr auto C6X_MAGICNUM_FH = 0x0099;          // in COFF 2 file header, indicates C6x-compatible COFF
constexpr auto C4X_MAGICNUM_FH = 0x0093;          // in COFF 2 file header, indicates C4x-compatible COFF

#define COFFBUFSZ_BYTES (768 * sizeof( ULONG ))   // must be a multiple of 12

constexpr auto COFF2_MAGICNUM = 0x00c2;           // file header field:  indicates COFF version 2 file
constexpr auto F_EXEC = 0x0002;                   // file header flags:  COFF file is executable
constexpr auto F_LITTLE = 0x0100;                 //    DSP target is a little-endian device
constexpr auto F_BIG = 0x0200;                    //    DSP target is a big-endian device
constexpr auto F_VERS1 = 0x0010;                  //    (C4x only) contains TMS320C40/44 code
constexpr auto COFFOPTFH_MAGICNUM = 0x0108;       // opt file hdr field: "magic number" for DOS-based COFFs
constexpr auto STYP_REG = 0x0000;                 // sect hdr flags: regular section (downloaded)
constexpr auto STYP_DSECT = 0x0001;               //    dummy section (not downloaded)
constexpr auto STYP_NOLOAD = 0x0002;              //    no-load section (not downloaded)
constexpr auto STYP_GROUP = 0x0004;               //    grouped section
constexpr auto STYP_PAD = 0x0008;                 //    padding section (downloaded)
constexpr auto STYP_COPY = 0x0010;                //    copy section (downloaded)
constexpr auto STYP_TEXT = 0x0020;                //    section contains executable code
constexpr auto STYP_DATA = 0x0040;                //    section contains initialized data
constexpr auto STYP_BSS = 0x0080;                 //    section contains uninitialized data
constexpr auto STYP_ALIGN = 0x0700;               //    (C4x only) section is aligned on page boundary
constexpr auto STYP_CLINK = 0x4000;               //    (C6x only) section requires conditional linking
...
//
constexpr auto COFF_FILE_HDR_SZ = 22;             // size of file header in bytes 
constexpr auto COFF_OPTFILE_HDR_SZ = 28;          // size of optional file header in bytes
constexpr auto COFF_SECTION_HDR_SZ = 48;          // size of section header in bytes
constexpr auto COFF_CINITSUB_HDR_SZ = 8;          // size of .cinit subsection header in bytes
...
constexpr auto C6X_DATABYTES = 1;                        // size of a data element in bytes for C6x-compatible COFF
constexpr auto C4X_DATABYTES = 4;                        // size of a data element in bytes for C4x-compatible COFF
...
BOOL RTFCNDCL CDevice::LoadTIDeviceCOFF()
{
   ...
   lpBuffer = (char *) ::RtAllocateLocalMemory( COFFBUFSZ_BYTES );           // alloc buf for reading data from file 
   ...
   {                                                                          //    orders in case file was created on 
      EndianSwap16( (PWORD) (&(fileHeader.version)), 2 );                     //    an alternate-endian system.
      EndianSwap32( (PDWORD)  (&(fileHeader.timeStamp)), 3 );
      EndianSwap16( (PWORD) (&(fileHeader.nOptHdrSize)), 3 );
   ...
   {
      EndianSwap16( (PWORD) (&(optFileHeader.magicnum)), 2 );
      EndianSwap32( (PDWORD)  (&(optFileHeader.nCodeSize)), 6 );
   }
   ...
   for( nSections = 0; nSections < fileHeader.nSectionHdrs; nSections++ )     // for each section in file...
   {
      ...
      if( bEndianSwapHeaders )                                                //    swap byte-order if necessary
      {
         ...
         EndianSwap32( (PDWORD)  (&(sectionHeader.physAddr)), 9 );
         EndianSwap16( (PWORD) (&(sectionHeader.reserved)), 2 );
      }
      ...
      while( (LONG)nSectBytesDone < sectionHeader.nSize )                     //    download one chunk at a time....
      {
         ...
         {                                                                    //    .cinit COPY section:
            ...
            if( bEndianSwapData )                                             //       endian-swap header if necessary 
               EndianSwap32( (PDWORD) (&(cInitSubHeader.nDataSize)), 2 );
         ...
         if( bEndianSwapData )                                                //    endian-swap data if necessary 
            EndianSwap32( (PDWORD) lpBuffer, int( dwBytesToRead>>2 ) );       //    (#bytes read should be mult of 4) 
...
CLEANUP:
   ...
   if( lpBuffer != (char *) NULL ) ::RtFreeLocalMemory( (PVOID)lpBuffer );
...
//    RETURNS:    NONE.
//
VOID RTFCNDCL CDevice::EndianSwap16( PWORD addr, int size )
{
...
VOID RTFCNDCL CDevice::EndianSwap32( PDWORD addr, int size )
{
   for( int index = 0; index < size; index++ )
   {
