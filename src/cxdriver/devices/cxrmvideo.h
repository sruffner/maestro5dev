...
constexpr int RMV_MAXCMDSIZE = 2053;   // longest command/reply (# of 32bit ints) we can send to/receive from RMVideo
...
public:
   ...
   double RTFCNDCL GetFramePeriod() const;                     // get RMVideo monitor frame period in sec, with ns precision
   int RTFCNDCL GetScreenW_pix() const;                        // get RMVideo monitor display width, in pixels
   int RTFCNDCL GetScreenH_pix() const;                        // get RMVideo monitor display height, in pixels

   int RTFCNDCL GetNumModes() const;                           // get number of available video display modes
   BOOL RTFCNDCL GetModeInfo(int n,                      // get display mode info: w x h in pixels, frame rate in Hz
         int& w, int& h, int& rate) const;
   int RTFCNDCL GetCurrentMode() const;                        // get/set the current video display mode
   ...
   double RTFCNDCL GetDegToPix() const;                        // retrieve deg->pix conversion factor
   double RTFCNDCL GetScreenW_deg() const;                     // get RMVideo monitor display width, in deg subtended at eye
   double RTFCNDCL GetScreenH_deg() const;                     // get RMVideo monitor display height, in deg subtended at eye

   VOID RTFCNDCL GetMonitorGamma(int &r, int&g, int &b) const; // get/set RMVideo monitor gamma correction factors
   BOOL RTFCNDCL SetMonitorGamma(int r, int g, int b);
   
   // get/set RMVideo vertical sync spot flash settings
   int RTFCNDCL GetSyncFlashSpotSize() const { return(m_syncFlashSize); }
   int RTFCNDCL GetSyncFlashDuration() const { return(m_syncFlashDur); }
   ...
   int RTFCNDCL GetNumTargets() const;                         // get number of targets currently defined
