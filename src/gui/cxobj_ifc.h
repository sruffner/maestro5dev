...
const WORD     CX_XYTARG            = 0x001C;         // [U,D] an individual XY scope target
...
const int   SGH_MINXYFRAME = 2;        // XY frame interval range; also must be a multiple of minimum value!
...
const int   STIM_ISXYSEQ      = 2;     //    specialized random-motion sequence on a set of XY scope targets; only one
...
const int   STIM_NXYSEQMODES  = 4;     // motion modes applicable to the XYSEQ stimulus type:
const int   MODE_ISSPARSEDIR  = 0;     //    direction randomized.  one XY tgt, randomly chosen, moves per "segment"
...
typedef struct tagRun               // definition of a ContMode run in a MAESTRODRIVER-compatible form
{
