...
// 
// ******************************
// Modifications to "official release".  saruffner.
// 29sep2000-- OnRButtonUp now sends NM_RCLICK to parent with coords of the 
//             **actual cell clicked**.  This allows parent to respond to right 
//             clicks on fixed cells.  Focus is still transferred as needed to 
//             a non-fixed cell.
//          -- GetCellFromPt() still works now if there are no non-fixed cells.
// 20nov2000-- Added member variables m_nMinColWidth, m_nMinRowHeight which 
//             specify the minimum allowed column width and row height.  These 
//             retrieved/modified with Get/SetMinColWidth/MinRowHeight().  They 
//             affect OnLButtonUp() when the WM_LBUTTONUP event ends a resizing 
//             operation. 
// 27nov2000-- Other member functions modified to handle the min col width and 
//             row height attributes...
// 05dec2000-- In virtual mode, the original framework did not provide a means for 
//             displaying different text in a cell's title tip vs the cell itself.  
//             In virtual mode, a static CGridCell is initialized with the cell info 
//             obtained from the grid callback function; and CGridCell::GetTipText() 
//             merely calls GetText(), returning the cell's contents.  To get around 
//             this problem, I added a cell state flag GVIS_VIRTUALTITLETIP (see 
//             GridCellBase.h) which can serve as a signal to the grid callback that 
//             the grid is querying for the cell's title tip text.  Functions GetCell() 
//             and OnMouseMove() also had to be modified to implement the feature.
//          -- The state flag GVIS_VIRTUALTITLETIP has another usage:  If a title tip 
//             fits within the cell's current rect, the tip is never displayed.  I've 
//             modified CTitleTip::Show() to provide a means of overriding that 
//             behavior:  a flag is set to TRUE if the tip should be displayed 
//             regardless.  In CGridCtrl::OnMouseMove(), we set this flag to TRUE if 
//             the state flag GVIS_VIRTUALTITLETIP is set for the cell.
// 04nov2002-- Began modifications that will allow a fixed cell in the grid to be 
//             the "focus" cell.  This feature is disabled by default -- since it 
//             would not be desirable in most applications.  Use SetAllowFixedFocusCell() 
//             to enable/disable the feature.  Note that we still cannot "select" a fixed
//             cell.  This feature is geared toward specialized usage of the grid ctrl -- 
//             such as when we want to a multi-row column header in view at all times, and 
//             some of the cells in that header contain modifiable data...
// 24aug2011-- Various changes to fix VStudio 10 compiler errors. The VS10 compiler has
// significant improvements in C++ standards conformance. The most typical error fixed had
// to do with the fact that variables declared in a for loop [ for(int i= ...) ] exist only
// in the scope of that loop. Also had to do some explicit casts to deambiguate two calls
// to an overloaded function. The biggest change was to rename the CMemDC class as 
// CGridMemDC (see also gridmemdc.h) because there's now an internal MFC class called CMemDC, 
// resulting in a linkage error (LNK2005).
// 31aug2017-- Various changes to fix VStudio 17 compiler errors. Now targeting 64-bit Win 10.
// Some changes have to do with conversions from "INT_PTR" to int, which could lose data if the
// integer value exceeds the maximum for a 32-bit signed int. 
/////////////////////////////////////////////////////////////////////////////
...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_nMinColWidth        = 0;          // 20nov2000(SAR) -- Minimum row/col dimensions
    m_nMinRowHeight       = 0;          //
    ...
    m_bAllowFixedFocus    = FALSE;      // 04nov2002(SAR)-- do NOT allow fixed cell to get the focus 
...
// 27nov2000(SAR)-- Modified to account for min col width, row height attributes.
//
void CGridCtrl::SetupDefaultCells()
{
   ...
   if( m_cellDefault.GetWidth() < m_nMinColWidth )
      m_cellDefault.SetWidth( m_nMinColWidth );
   if( m_cellDefault.GetHeight() < m_nMinRowHeight )
      m_cellDefault.SetHeight( m_nMinRowHeight );
   ...
   if( m_cellFixedColDef.GetWidth() < m_nMinColWidth )
      m_cellFixedColDef.SetWidth( m_nMinColWidth );
   if( m_cellFixedColDef.GetHeight() < m_nMinRowHeight )
      m_cellFixedColDef.SetHeight( m_nMinRowHeight );
   ...
   if( m_cellFixedRowDef.GetWidth() < m_nMinColWidth )
      m_cellFixedRowDef.SetWidth( m_nMinColWidth );
   if( m_cellFixedRowDef.GetHeight() < m_nMinRowHeight )
      m_cellFixedRowDef.SetHeight( m_nMinRowHeight );
   ...
   if( m_cellFixedRowColDef.GetWidth() < m_nMinColWidth )
      m_cellFixedRowColDef.SetWidth( m_nMinColWidth );
   if( m_cellFixedRowColDef.GetHeight() < m_nMinRowHeight )
      m_cellFixedRowColDef.SetHeight( m_nMinRowHeight );
}
...
// 04nov2002(SAR)-- Modified to allow focus to move to a fixed cell, IF that feature is enabled.
//
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
   ...
   int iFirstRow = GetAllowFixedFocus() ? 0 : GetFixedRowCount();          // 04nov2002(SAR)-- Let focus move to fixed 
   int iFirstCol = GetAllowFixedFocus() ? 0 : GetFixedColumnCount();       // cell, but only if feature is enabled. 
    ...
    else if (nChar == VK_UP)
    {
        ...
        while( next.row >= iFirstRow )                                                       // 04nov2002(SAR)
        {
            if( GetRowHeight( next.row) > 0)
            {
                bFoundVisible = TRUE;
    ...
    else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
    {
        ...
        if (nChar == VK_TAB)
        {
            ...
            {
                next.row++; 
                next.col = iFirstCol;                                                        // 04nov2002(SAR)
                bChangeLine = TRUE;
            }
            else
                CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
    ...
    else if (nChar == VK_LEFT || (nChar == VK_TAB && IsSHIFTpressed()) )
    {
        ...
        if (nChar == VK_TAB)
        {
            if( next.col < iFirstCol && next.row > iFirstRow )                               // 04nov2002(SAR)
            {
                next.row--; 
        ...
        while( next.col >= iFirstCol )                                                       // 04nov2002(SAR)
        {
            if( GetColumnWidth( next.col) > 0)
            {
                bFoundVisible = TRUE;
    ...
    else if (nChar == VK_PRIOR)
    {
        ...
        {
            next.row += increment;
            if (next.row < iFirstRow )                                                       // 04nov2002(SAR)
                next.row = iFirstRow;
        }
        else
            next.row = iFirstRow;                                                            // 04nov2002(SAR)
    }
...
// 04nov2002(SAR) -- Modified to allow fixed cell to get the focus, IF the feature is enabled
//
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
   ...
   if( !GetAllowFixedFocus() )                                                               // 04nov2002(SAR)
   {
      ...
      if( cell == m_idCurrentCell ) return( m_idCurrentCell );                               // 13nov2002(SAR)
   }
...
//=== SetAllowFixedFocus ============================================================================================== 
//
//    04nov2002(SAR) -- Enable/disable feature allowing a fixed cell in the grid to become the focus cell.
//
//    ARGS:       bAllow   -- [in] if TRUE, we allow fixed cell to be the focus cell.
//
//    RETURNS:    NONE.
//    
void CGridCtrl::SetAllowFixedFocus( BOOL bAllow )
{
   BOOL bOld = m_bAllowFixedFocus;
   m_bAllowFixedFocus = bAllow;

   if( bOld && !m_bAllowFixedFocus )                              // if turning off feature, be sure to adjust the 
   {                                                              // focus cell if it is currently on a fixed cell!
      CCellID c = m_idCurrentCell;
      if( IsValid( c ) && IsCellFixed( c.row, c.col ) )
      {
         if( c.row < GetFixedRowCount() ) c.row = GetFixedRowCount();
         if( c.col < GetFixedColumnCount() ) c.col = GetFixedColumnCount();
         SetFocusCell( c );
      }
   }
}
...
// 29sep2000-- SAR. If there are not yet any non-fixed cells, this fcn fails 
// even if we click a fixed cell and bAllowFixedCellCheck is TRUE.  FIXED.
//
// Get cell from point
CCellID CGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
    ...
    else if ( idTopLeft.col < 0 )      // not in fixed col, and there are no non-fixed cells yet
        cellID.col = -1;
    else    // in non-fixed col, or beyond
    {
    ...
    else if ( idTopLeft.row < 0 )      // not in fixed row, and there are no non-fixed cells yet
        cellID.row = -1;
    else    // in non-fixed row, or beyond
    {
...
// 
// 27nov2000-- Modified to implement min col width attribute
//
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    if ( m_arColWidths[nColumn] < (UINT) m_nMinColWidth )
         m_arColWidths[nColumn] = (UINT) m_nMinColWidth;
...
// 
// 27nov2000-- Modified to implement min row height attribute
//
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    if ( m_arRowHeights[nRow] < (UINT) m_nMinRowHeight )
         m_arRowHeights[nRow] = (UINT) m_nMinRowHeight;
...
// 20nov2000(SAR)-- Added methods to implement minimum column width and row height attributes. 
//
// Setting the minimum value to 0 effectively turns off the feature.  Largest allowable 
// minimum dimension is 500.
//
int CGridCtrl::SetMinColWidth( int minD )
{ 
   if ( (minD < 0) || (minD > 500) ) return( -1 );

   int oldD = m_nMinColWidth;
   m_nMinColWidth = minD;

   if ( m_nMinColWidth <= oldD ) return( oldD );      // no adjustments necessary

   // adjust current default cell definitions
   //
   if( m_cellDefault.GetWidth() < m_nMinColWidth )
      m_cellDefault.SetWidth( m_nMinColWidth );
   if( m_cellFixedColDef.GetWidth() < m_nMinColWidth )
   ...
   // adjust any existing columns, if necessary.
   //
   BOOL bAdjusted = FALSE;
   for( int i = 0; i < GetColumnCount(); i++ )
   {
      if( m_arColWidths[i] < (UINT) m_nMinColWidth )
      {
         m_arColWidths[i] = (UINT) m_nMinColWidth;
         bAdjusted = TRUE;
      }
   }

   // if an adjustment was made, redraw entire grid
   //
   if( bAdjusted )
   {
      Refresh();
...
int CGridCtrl::SetMinRowHeight( int minD )
{ 
   if ( (minD < 0) || (minD > 500) ) return( -1 );

   int oldD = m_nMinRowHeight;
   m_nMinRowHeight = minD;

   if ( m_nMinRowHeight <= oldD ) return( oldD );     // no adjustments necessary
   ...
   // adjust any existing rows, if necessary.
   //
   BOOL bAdjusted = FALSE;
   for( int i = 0; i < GetRowCount(); i++ )
   {
      if( m_arRowHeights[i] < (UINT) m_nMinRowHeight )
      {
         m_arRowHeights[i] = (UINT) m_nMinRowHeight;
         bAdjusted = TRUE;
      }
   }
...
// 27nov2000-- Modified to implement minimum row height attribute.
//
BOOL CGridCtrl::SetRowHeight(int nRow, int height)
{
   ...
   m_arRowHeights[nRow] = (height < m_nMinRowHeight) ? m_nMinRowHeight : height;
   ResetScrollBars();
   return( TRUE );
}

// 27nov2000-- Modified to implement minimum column width attribute.
//
BOOL CGridCtrl::SetColumnWidth(int nCol, int width)
{
   ...
   m_arColWidths[nCol] = (width < m_nMinColWidth) ? m_nMinColWidth : width;
   ResetScrollBars();
   return( TRUE );
}
...
// 27nov2000-- Modified to implement minimum col width attribute.
//
...
//
// 27nov2000-- Modified to implement minimum row height and minimum col width attributes.
//
void CGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    ...
    {
        if( GetRowHeight( nRow) > 0 )
            m_arRowHeights[nRow] = (m_nMinRowHeight>1) ? m_nMinRowHeight : 1;
    }
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
    {
        ...
        if( GetColumnWidth( nCol) > 0 )
        {
            ...
            m_arColWidths[nCol] = m_nMinColWidth;
            for (nRow = nStartRow; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
...
void CGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    ...
    // column widths may not shrink below current min column width
    //
    for (int i = nFirstColumn; i < GetColumnCount(); i++)
    {
        if ( m_arColWidths[i] < (UINT) m_nMinColWidth )
...
void CGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
    ...
    // row heights may not shrink below current min row height
    //
    for (int i = nFirstRow; i < GetRowCount(); i++)
    {
        if ( m_arRowHeights[i] < (UINT) m_nMinRowHeight )
...
BOOL CGridCtrl::IsCellVisible(int nRow, int nCol)
{
   ...
   if( !IsValid( nRow, nCol ) ) return( FALSE );                        // 15nov2002(sar): minor bug fix
        ...
        (m_bMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING)
        {
            ...
            if (!GetVirtualMode() || m_bTitleTips)
            {
                ...
                // over it.
                // 05dec2000(SAR) -- Modified to permit getting title tip text that's different from 
                //   cell's contents, even in virtual mode; note the boolean TRUE arg to GetCell()!
                idCurrentCell = GetCellFromPt(point);
                pCell = GetCell(idCurrentCell.row, idCurrentCell.col, TRUE );
                if (pCell)
                    pCell->OnMouseOver();
            }
                ...
                if (pCell)
                {
                        ...
                        && GetCellRect(idCurrentCell.row, idCurrentCell.col, CellRect) )
                    {
                        m_TitleTip.Show(TextRect, pCell->GetTipText(),  0, CellRect,
                                        ...
                                        ((pCell->GetState() & GVIS_VIRTUALTITLETIP) != 0));
                    }
                }
            }
#endif
...
// 04nov2002(SAR)-- FYI: The "allow fixed focus cell" feature could circumvent the code below -- since SetFocusCell() has 
// has been modified to allow a fixed cell to get the focus if the feature is enabled.
...
// 04nov2002(SAR)-- Modified to permit initiation of inplace editing of fixed cells by a left dbl-click IF the "allow 
// fixed focus cell" feature is enabled...
//
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    TRACE("CGridCtrl::OnLButtonDblClk\n");
...
//  ignore rows that are hidden and look upt towards first visible row
    ...
    else if (m_MouseMode == MOUSE_NOTHING)
    {
        ...
        int iFirstRow = GetAllowFixedFocus() ? 0 : GetFixedRowCount();                          // 04nov2002(SAR)
        int iFirstCol = GetAllowFixedFocus() ? 0 : GetFixedColumnCount();

        if (cell.row >= iFirstRow && IsValid(m_LeftClickDownCell) && cell.col >= iFirstCol)
        {
        ...
        else if (m_bListMode)
        {
            if (!IsValid(cell))
                return;
            if (cell.row >= iFirstRow && cell.col < GetFixedColumnCount())                      // 04nov2002(SAR) 
                OnEditCell(cell.row, cell.col, pointClickedRel, VK_LBUTTON);
        }
...
// 04nov2002(SAR)-- Allow focus to switch to a fixed cell when the user mousedowns on it -- but only if the "allow 
// fixed focus cell" feature is enabled.
//
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
             ...
             m_MouseMode != MOUSE_OVER_ROW_DIVIDE)        
    {
        ...
        SetFocusCell( m_LeftClickDownCell );    // 04nov2002(SAR)-- Code below replaced to allow focus to move to a 
                                                // fixed cell if that feature is enabled.  See SetFocusCell().
/* 
...
// SAR - (29sep2000) Previous version did not allow fixed cells to be right-clicked.
// Modified so that NM_RCLICK is sent for the actual cell that is 
// right-clicked, whether it is fixed or not.  Focus is still moved to 
// the non-fixed cell implied by right-clicking a fixed cell
// 04nov2002(SAR)-- Focus goes to a rt-clicked fixed cell, IF "allow fixed focus cell" feature is enabled.
//
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
   CWnd::OnRButtonUp( nFlags, point );
   CCellID rClickCell = GetCellFromPt( point );

   EndEditing();        // Auto-destroy any InPlaceEdit's

   // If not a valid cell, pass -1 for row and column
   if( !IsValid( rClickCell ) )
      SendMessageToParent( -1, -1, NM_RCLICK );
   else
   {
      SetFocusCell( -1, -1 );                            // clear previous focus

      SetFocusCell( rClickCell );                        // 04nov2002(SAR)-- Allow focus on fixed cell, if approp.
//    SetFocusCell( max(rClickCell.row, m_nFixedRows),   // set focus to non-fixed cell
//                  max(rClickCell.col, m_nFixedCols) );


      // if new focus cell is different from cell actually r-clicked, 
      // give the cell actually clicked a chance to do something
      if( rClickCell != m_idCurrentCell )                                                    // 04nov2002(SAR)
      {
         CGridCellBase* pCell = GetCell( rClickCell.row, rClickCell.col );
         if ( pCell )
            pCell->OnRClick( GetPointClicked( rClickCell.row, rClickCell.col, point ) );
      }

      // give new focus cell a chance to do something
      CGridCellBase* pCell = GetCell( m_idCurrentCell.row, m_idCurrentCell.col );
      if ( pCell )
         pCell->OnRClick( GetPointClicked( m_idCurrentCell.row, m_idCurrentCell.col, point ) );

      // 29nov2000(SAR) parent receives the cell coords of **actual cell clicked**
      SendMessageToParent( rClickCell.row, rClickCell.col, NM_RCLICK );
   }
}
#endif
