...
public:
    ...
    void     SetDefCellHeight(int nHeight)   { int h = (nHeight < m_nMinRowHeight) ? m_nMinRowHeight : nHeight;
                                               m_cellDefault.SetHeight(h); 
                                               m_cellFixedRowDef.SetHeight(h); 
                                               m_cellFixedColDef.SetHeight(h); 
                                               m_cellFixedRowColDef.SetHeight(h); }
    int      GetDefCellWidth() const         { return m_cellDefault.GetWidth();     }
    void     SetDefCellWidth(int nWidth)     { int w = (nWidth < m_nMinColWidth) ? m_nMinColWidth : nWidth;
                                               m_cellDefault.SetWidth(w); 
                                               m_cellFixedRowDef.SetWidth(w); 
                                               m_cellFixedColDef.SetWidth(w); 
                                               m_cellFixedRowColDef.SetWidth(w); }

    // ***************************************************************************** //

    int GetSelectedCount() const  { return( static_cast<int>(m_SelectedCellMap.GetCount()) ); }

    CCellID SetFocusCell(CCellID cell);
    CCellID SetFocusCell(int nRow, int nCol);
    CCellID GetFocusCell() const                  { return m_idCurrentCell;           }

    void SetAllowFixedFocus( BOOL bAllow );                                               // 04nov2002(SAR)
    BOOL GetAllowFixedFocus() const               { return( m_bAllowFixedFocus ); }       //
   ...
   int  GetMinColWidth() const                    { return m_nMinColWidth; } // 20nov2000(SAR) -- Added features. 
   int  GetMinRowHeight() const                   { return m_nMinRowHeight; }
   int  SetMinColWidth( int minD );
   int  SetMinRowHeight( int minD );
...
public:
    // 05dec2000(SAR)-- Added arg bTitleTip.  See inline implementation...
    CGridCellBase* GetCell(int nRow, int nCol, BOOL bTitleTip = FALSE ) const;   // Get the actual cell!
...
protected:
    ...
    BOOL        m_bAllowFixedFocus;                               // 04nov2002(SAR) -- if TRUE, a fixed cell can 
                                                                  // receive the focus.
    int         m_nGridLines;
    BOOL        m_bEditable;
    BOOL        m_bModified;
    ...
    int         m_nMinColWidth, m_nMinRowHeight;                  // 20nov2000(SAR)
...
// 05dec2000(SAR)-- Added argument bTitleTip.  This is used only in virtual mode.  If TRUE, 
// its effect is to set the GV_ITEM.nState flag GVIS_VIRTUALTITLETIP, an indication to the grid callback 
// function that we're asking for the cell's title tip text rather than its normal contents.  This allows 
// applications running the grid in virtual mode to provide title tips that are different from the cell's 
// normal text content!
inline CGridCellBase* CGridCtrl::GetCell(int nRow, int nCol, BOOL bTitleTip /* FALSE */ ) const
{
    ...
    if (GetVirtualMode())
    {
        ...
        if ( bTitleTip ) gvdi.item.nState |= GVIS_VIRTUALTITLETIP;
