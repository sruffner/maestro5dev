...
//
BOOL CCxRuntime::IsEmptyMessageQueue() const
{
...
//
WORD CCxRuntime::GetTraces() const
{
   int iMode = GetMode();
...
//    long for a response, we sleep for ~1ms intervals using Sleep(1).
//
...
//    18sep2024(sar) -- Removed use of RtSleepFt. As of RTX64 4.0, it is no longer supported on the Windows side.
//
...
//
int CCxRuntime::ConvertVoltsToRaw( float fVolt, BOOL isAI ) const
{
...
float CCxRuntime::ConvertRawToVolts( int iDac, BOOL isAI ) const
{
   DWORD resFlagBit = isAI ? CX_F_AI16BIT : CX_F_AO16BIT;
   BOOL use16Bit = BOOL((GetHWStatus() & resFlagBit) == resFlagBit);
...
BOOL CCxRuntime::Open()
{
   ...
   m_hRtShared = CCxRtapi::RtCreateSharedMemory(SHM_MAP_ALL_ACCESS, 0, sizeof(CXIPCSM), CXIPC_SHM, (VOID **) &m_pShm); 
   if(m_hRtShared == (HANDLE) NULL)
   {
