...
// 06nov2019-- Fixing compiler warnings when trying to build RMVideo on Lubuntu 18.04 using g++ 7.4.0...
//=====================================================================================================================
...
bool CRMVMediaMgr::load()
{
   ...
   while(pDirEntry != NULL)
   {
      ...
      if(append)
      {
         ::sprintf(dirPath, "%.50s/%.100s", MEDIASTOREDIR, pDirEntry->d_name);
...
{
   char path[255];
   ::sprintf(path, "%.50s/%.100s", MEDIASTOREDIR, pFolder->name);
   ...
   while(pDirEntry != NULL)
   {
      ...
      if(append)
      {
         ::sprintf(path, "%.50s/%.100s/%.100s", MEDIASTOREDIR, pFolder->name, pDirEntry->d_name);
