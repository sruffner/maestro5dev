/*=====================================================================================================================
 ...
 CRMVTarget::updateMotion() is called on that thread and is responsible for reading in the next movie frame from file 
 ...
 in Jan 2020 revealed that running the master thread at SCHED_FIFO priority was causing occasional multi-second freezes
 in the NVidia driver's implementation of glFinish() [with VSync enabled] and in the processing of network receive
 packets. With RMVideo's main thread at normal SCHED_OTHER priority, these "hangs" disappeared. As a result, the
 ...
 **************** SUMMARY OF PERFORMANCE TESTING **********************************************************************
 ...
 1/27/20: Further testing in Jan 2020 found: (1) It is critical that NVidia's PowerMizer mode be set to "Prefer maximum
 ...
 master thread at normal SCHED_OTHER priority eliminated these hangs. As a consequence, it is no longer necessary to
 modify the processor affinities of the master thread and the CVidBuffer worker thread in initialize().
 **************** SUMMARY OF PERFORMANCE TESTING **********************************************************************
 ...
 06nov2019-- Working on changes to compile RMVideo under Lubuntu 18.04 LTS, which uses newer versions of the LIBAV*
 libraries, sourced from FFMPEG project rather than LIBAV. A lot of deprecation warnings to address, as well as some
 errors (API not present in FFMPEG version of LIBAV libraries).
 20jan2020-- Discovered that FFMPEG library was not using multi-threaded decoding, when supported by the codec, to
 process video files. As a result CVidBuffer could not keep up streaming 1920x1080 video under Lubuntu 18.04 (in 
 RMVideo under 14.04 using the LIBAV library, multi-threaded decoding was in use). Modified openVideoStream() to fix.
 27jan2020-- Removed code in initialize() that modified processor affinities of RMVideo's main thread and CVidBuffer's
 worker thread, so the main thread now runs with normal SCHED_OTHER priority. See DESCRIPTION and PERFORMANCE TESTING.
 28jan2020-- Suppress all log messages from FFMPEG library via call to av_log_set_level() in getVideoInfo(), which is
 called early during RMVideo start-up.
//===================================================================================================================*/
...
bool CVidBuffer::getVideoInfo(const char* path, int& w, int& h, int& r, int& d, bool quiet)
{
   ...
   // suppress all logging messages from FFMPEG library
   int level = av_log_get_level();
   if(level != AV_LOG_QUIET)
   {
      ::fprintf(stderr, "[NOTE: Suppressing all log messages from FFMPEG. Log level was %d.]\n", level);
   ...
   int iStream = -1;
   for(int i=0; i < pFormatCtx->nb_streams; i++) if(pFormatCtx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
   {
      iStream = i;
      break;
   }
   ...
   // find the codec that we'll need to decode the video stream. 
   ...
   // allocate codec context, copy codec parameters to it, then initialize context to use the given codec
   AVCodecContext* pCodecCtx = avcodec_alloc_context3(NULL);
   bool ok = (pCodecCtx != NULL);
   if(ok) ok = (avcodec_parameters_to_context(pCodecCtx, pStream->codecpar) >= 0);
   if(ok) ok = (avcodec_open2(pCodecCtx, pCodec, NULL) >= 0);
   if(!ok)
   {
      if(pCodecCtx != NULL) avcodec_free_context(&pCodecCtx);
      avformat_close_input(&pFormatCtx);
      if(!quiet) ::fprintf(stderr, "ERROR(CVidBuffer): Failed to open/init codec context\n");
      return(false);
   }
...
/**
 ...
 as the calling thread, which should be RMVideo's main thread. [It is assumed both threads run with nomral 
...
int CVidBuffer::openVideoStream(const char* path, bool preload, bool stopOnEOF)
{
   ...
   for(int i=0; i < pStream->pFmtCtx->nb_streams; i++) 
      if(pStream->pFmtCtx->streams[i]->codecpar->codec_type==AVMEDIA_TYPE_VIDEO)
   {
      pStream->streamIdx = i;
      break;
   }
   ...
   // find the codec that we'll need to decode the video stream
   AVCodecParameters* pCodecPar = pStream->pFmtCtx->streams[pStream->streamIdx]->codecpar;
   AVCodec *pCodec = avcodec_find_decoder(pCodecPar->codec_id);
   if(pCodec == NULL)
   {
      ::fprintf(stderr, "ERROR(CVidBuffer): Video file %s uses unsupported codec (id=%d)\n", path,
               (int) (pCodecPar->codec_id));
      closeVideoStream(pStream);
      return(-4);
   }
   ...
   // allocate codec context, copy codec parameters to it, then initialize context to use the given codec
   pStream->pCodecCtx = avcodec_alloc_context3(NULL);
   bool ok = (pStream->pCodecCtx != NULL);
   if(ok) ok = (avcodec_parameters_to_context(pStream->pCodecCtx, pCodecPar) >= 0);

   // IMPORTANT! avcodec_alloc_context3() initializes thread_count to 1, so only one worker thread is used for decoding.
   // Here we set thread_count to 0, telling the library to decide how many threads to use!
   // TESTING NOTE: This works, and thread_count is typically set to #CPUS + 1 by avcodec_open2() -- at least for codecs
   // that support multi-threaded decoding. Testing showed that it is critical to use multi-threaded decoding in order
   // to handle larger videos. Without it, CVidBuffer could not stream 1920x1080 video fast enough.
   pStream->pCodecCtx->thread_count = 0;

   if(ok) ok = (avcodec_open2(pStream->pCodecCtx, pCodec, NULL) >= 0);
   if(!ok)
   {
      ::fprintf(stderr, "ERROR(CVidBuffer): %s - Failed to open/init codec context\n", path);
   ...
   pStream->pSwsCtx = sws_getContext(w, h, pStream->pCodecCtx->pix_fmt, w, h, AV_PIX_FMT_RGB24, SWS_BICUBIC, 
         NULL, NULL, NULL);
   ...
   pStream->pDstFrame = av_frame_alloc();
   ok = (pStream->pDstFrame != NULL);
   pStream->nBytes = av_image_get_buffer_size(AV_PIX_FMT_RGB24, w, h, 1);
...
/**
 ...
 stream, converting it to the AV_PIX_FMT_RGB24 format, and storing it in the next available slot in the stream's buffer
...
void CVidBuffer::readNextVideoFrame(VideoStream* pStream)
{
   ...
   av_image_fill_arrays(pStream->pDstFrame->data, pStream->pDstFrame->linesize, 
         pStream->frameQueue[pStream->iWrite], AV_PIX_FMT_RGB24, pStream->width, pStream->height, 1);
   ...
   while(!(pStream->disabledOnError || gotFrame))
   {
      ...
      if(packet.stream_index == pStream->streamIdx) 
      {
         ...
         // DEPRECATED: avcodec_decode_video2(pStream->pCodecCtx, pSrcFrame, &frameFinished, &packet);
         bool frameDone = (0 == avcodec_send_packet(pStream->pCodecCtx, &packet));
         if(frameDone) frameDone = (0 == avcodec_receive_frame(pStream->pCodecCtx, pSrcFrame));
   ...
   if(pSrcFrame != NULL) av_frame_free(&pSrcFrame);
}
...
void CVidBuffer::closeVideoStream(VideoStream* pStream)
{
   ...
   {
      av_frame_free(&(pStream->pDstFrame));
      pStream->pDstFrame = NULL;
   }
